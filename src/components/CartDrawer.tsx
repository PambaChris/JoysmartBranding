...
export function CartDrawer() {
  ...
  const handleCheckout = () => {
    ...
    const total = `\n\n*Total: ${items[0]?.price.currencyCode} ${totalPrice.toFixed(2)}*`;
    const finalMessage = encodeURIComponent(`Hello Joysmart, I would like to order:\n\n${message}${total}`);
          ...
          {items.length === 0 ? (
            ...
            <>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t bg-background">
                ...
                <Button onClick={handleCheckout} className="w-full bg-[#25D366] hover:bg-[#128C7E] text-white" size="lg" disabled={items.length === 0 || isLoading}>
                  {isLoading ? (
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ) : (
                    <>
                      <img src="/Whatsapp.png" alt="WhatsApp" className="w-6 h-6 mr-2 object-contain" />
                      Order via WhatsApp
                    </>
                  )}
                </Button>
